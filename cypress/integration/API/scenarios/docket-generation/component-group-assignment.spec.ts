...
/**
 * This file consists of the test automation for sub po creation to docket generation with multiple component group assignment
 */
...
const style = Cypress.env("componentGroupStyle");
const schedule = Cypress.env("componentGroupSchedule");
const color = Cypress.env("componentGroupColor");
let selectedPoData = {
    ...
    it("Create Replacement Group", () => {
        const createReplacementGroup = {
            ...
            body: {
                components: ["FR"],
                createdUser: "dev",
                fabricCategory: "TESTFRT02",
                groupType: "REPLACEMENT",
                masterPoDetailsId,
                materialItemCode: "SFPFMENVFB1 004",
                plantCode: "ENV",
...
describe('9) Sub Ratio PO', () => {
    ...
    context('Select Sub PO Numbers', () => {
        ...
        it('Get PO Summary and Ratio CG Summary', () => {
            ...
            cy.request(options).then(response => {
                ...
                expect(response.body.data.componentGroupInfoForPO.ratios[0].cgName).to.equal("CG1")
                expect(response.body.data.componentGroupInfoForPO.ratios[0].fabricCategory).to.equal(fabricDetails[0].fabricCategory)
                expect(response.body.data.componentGroupInfoForPO.ratios[1].cgName).to.equal("CG2")
                expect(response.body.data.componentGroupInfoForPO.ratios[1].fabricCategory).to.equal(fabricDetails[1].fabricCategory)
    ...
    context('Add Ratio for PO', () => {
        ...
        it('Create Component Group Based Ratio', () => {
            ...
            // poSummaryData
            const totalQty = Util.calculateQty(poSummaryData);
            const quantitiesSubPo = Util.calculateSizeRatios(defaultSizes, totalQty);
