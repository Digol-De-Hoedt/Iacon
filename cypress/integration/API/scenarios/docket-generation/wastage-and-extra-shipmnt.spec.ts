...
/**
 * This file consists of the test automation for sub po creation to docket generation for wastage and extra shipment
 */
...
const style = Cypress.env("wastageStyle");
const schedule = Cypress.env("wastageSchedule");
const color = Cypress.env("wastageColor");
...
let subPoNumber = <string | null>null;
...
describe("1) Master PO Selection", () => {
    ...
    it("Get PO Related Schedules for Selected Style", () => {
        expect(style).to.be.a("string", "Selected style is available");
    ...
    it("Get PO Related Colors for Selected Style and Schedule", () => {
        expect(schedule).to.be.a("string", "Selected schedule is available");
        const getSchedulesForStyle = {
            method: "POST",
    ...
    it("Get All Master Production Orders for Style-Schedule-Color", () => {
        ...
        cy.request(getAllMasterProductionOrdersForStyleScheduleColor).then((response) => {
            ...
            cy.log(`Selected Master PO ${selectedPoData.selectedMasterPo['masterPoSeq']} [${masterPoNumber}]`)
        });
    });
});
...
describe("2) Operation Routing", () => {
    ...
    it("Get Versions for Style-Color from SMS", () => {
        ...
        cy.request(getVersionsForStyleColorFromSms).then((response) => {
            ...
            const versionDataMatchingSavedVersion = Array.from(response.body.data).filter(
                (i) => i["variant"] === selectedPoData.savedDefaultOperationVersion
            );
...
describe("7) Replacement group", () => {
    ...
    before(() => {
        ...
        cy.request(getReplacementGroupsByMasterPo).then(async (getReplacementGroupsResponse) => {
            ...
            if (
                getReplacementGroupsResponse.body.data &&
                getReplacementGroupsResponse.body.data.replacementGroupDetails &&
                getReplacementGroupsResponse.body.data.replacementGroupDetails.length > 0
            ) {
                await Promise.all(
                    getReplacementGroupsResponse.body.data.replacementGroupDetails.map((replacementGroup) => {
    ...
    it("Get RM Components Mapping For Styles Colors", () => {
        ...
        cy.request(getRmComponentsMappingForStylesColors).then((response) => {
            ...
            rmMappings = response.body.data[0].rmMapping
        });
    });
    ...
    it("Get Fabric Max Plies Details", () => {
        ...
        cy.request(getFabricMaxPliesDetails).then((response) => {
            ...
            expect(response.body.internalMessage).to.equal(Constants.PPS_DATA_FOUND);
            expect(response.body.data).to.be.an('array').and.have.length.greaterThan(0)
            maxPliesDetails = response.body.data[0]
        });
    });
    ...
    it("Create Replacement Group", () => {
        const createReplacementGroup = {
            ...
            body: {
                components: rmMappings[0].components,
                createdUser: "dev",
                fabricCategory: maxPliesDetails.fabricCategory,
                groupType: "REPLACEMENT",
                masterPoDetailsId,
                materialItemCode: rmMappings[0].rmSku,
                plantCode: "ENV",
                replacementGroupName: "123",
            },
        };
...
describe("8) Sub PO Creation", () => {
    ...
    it("get master PO quantity summery", () => {
        ...
        cy.request(getMasterPoQuantitiesSummary).then((response) => {
            ...
            expect(masterPoQuantitiesSummary.summaryData).to.be.an("object");
            expect(masterPoQuantitiesSummary.summaryData.ORIGINAL_QUANTITY).to.be.an("array");

            // ORIGINAL_QUANTITY
            // aggregate each size into key:value
            const aggregatedOriginalQuantity: { [id: string]: number } = {};
            for (let extraShipmentData of masterPoQuantitiesSummary.summaryData.ORIGINAL_QUANTITY) {
            ...
            for (let extraShipmentData of masterPoQuantitiesSummary.summaryData.EXTRA_SHIPMENT || []) {
            ...
            // SAMPLE
            // aggregate each size into key:value
            const aggregatedSample: { [id: string]: number } = {};
            for (let extraShipmentData of masterPoQuantitiesSummary.summaryData.SAMPLE || []) {
                if (aggregatedSample[extraShipmentData.size] >= 0) {
                    aggregatedSample[extraShipmentData.size] = aggregatedSample[extraShipmentData.size] + extraShipmentData.quantity;
                } else {
                    aggregatedSample[extraShipmentData.size] = extraShipmentData.quantity;
                }
            }
            //  compare aggregated result with input from additional qty screen
            for (let [size, sampleAmount] of Object.entries(aggregatedSample)) {
                expect(orderQuantitiesData.SAMPLE[size].quantity).to.be.equal(sampleAmount, `Sample quantity is correct for size ${size}`);
            }
        });
    });
    ...
    context("Add Ratio for PO", () => {
        ...
        it("Create Component Group Based Ratio", () => {
            ...
            const sizeRatios = qtyBySizes.map((item) => {
                let ratio = Math.ceil(item.qty / leastQty) * 1.1; // 110% more
                return {
                    size: item.size,
...
describe("12) Packing List", () => {
    ...
    it("Get MP Pack List Summary", () => {
        ...
        cy.request(getMpPackListSummary).then((response) => {
            expect(response.status).to.equal(201);
